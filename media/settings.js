(function () {
  ...
  const fields =
    (window.AgentDevice && window.AgentDevice.data && window.AgentDevice.data.fields) || [];
  let snapshot = [];
  ...
  function scopeBtn(value, label, disabled) {
    ...
    return (
      '<button class="scope-btn" data-scope="' +
      value +
      '"' +
      (disabled ? ' disabled' : '') +
      ' aria-selected="' +
  ...
  function renderField(field) {
    ...
    const sourceLabel = explicitlySet
      ...
      : otherValue !== undefined
        ? '<span class="field-source">inherited from ' +
    ...
    const resetBtn = explicitlySet
      ? '<button class="action" data-reset="' +
        esc(field.key) +
        '" title="Reset this scope">↺</button>'
      : '';
    return (
      '<div class="field">' +
      '<div class="field-label">' +
