(function () {
  ...
  function renderRun() {
    const back =
      '<div class="actions"><button id="back" class="ghost"><i class="codicon codicon-arrow-left"></i>Back to list</button></div>';
    ...
    if (state.status === 'running') {
      ...
      if (state.reportAvailable) {
        buttons.push(
          '<button id="open-report" class="ghost"><i class="codicon codicon-file-symlink-file"></i>View report</button>',
        );
      }
  ...
  function renderSummary() {
    ...
    const name = state.scriptName
      ? '<span class="script-name" id="reveal">' + esc(state.scriptName) + '</span>'
      : '';
    const dur =
      state.status === 'running'
        ? '<span class="duration" id="elapsed">0ms</span>'
        : state.durationMs != null
          ? '<span class="duration">' + formatDuration(state.durationMs) + '</span>'
          : '';
  ...
  function statusHtml(status) {
    if (status === 'running')
      return '<span class="status running"><i class="codicon codicon-loading codicon-modifier-spin"></i>Running</span>';
    if (status === 'success')
      return '<span class="status success"><i class="codicon codicon-pass-filled"></i>Pass</span>';
    if (status === 'failure')
      return '<span class="status failure"><i class="codicon codicon-error"></i>Fail</span>';
    if (status === 'cancelled')
      return '<span class="status cancelled"><i class="codicon codicon-circle-slash"></i>Cancelled</span>';
    return '';
  }

  function stepIconHtml(state) {
    if (state === 'pending')
      return '<span class="step-icon pending"><i class="codicon codicon-circle-large-outline"></i></span>';
    if (state === 'running')
      return '<span class="step-icon running"><i class="codicon codicon-loading codicon-modifier-spin"></i></span>';
    if (state === 'passed')
      return '<span class="step-icon passed"><i class="codicon codicon-pass-filled"></i></span>';
    if (state === 'failed')
      return '<span class="step-icon failed"><i class="codicon codicon-error"></i></span>';
    if (state === 'skipped')
      return '<span class="step-icon skipped"><i class="codicon codicon-circle-slash"></i></span>';
    return '';
  }
  ...
  function renderStep(step) {
    ...
    const header =
      '<div class="step-header" data-step="' +
      step.index +
      '">' +
      stepIconHtml(step.state) +
      '<span class="step-line" data-line="' +
      step.lineNumber +
      '">L' +
      step.lineNumber +
      '</span>' +
  ...
  function renderStepBody(step) {
    ...
    if (parts.length === 0) {
      ...
      parts.push(
        '<div class="step-body"><div class="label-row"><span class="label">' +
          placeholder +
          '</span></div></div>',
      );
    }
  ...
  function bind() {
    ...
    if (reportBtn)
      reportBtn.addEventListener('click', function () {
        vscode.postMessage({ type: 'open-report' });
      });

    const backBtn = document.getElementById('back');
    if (backBtn)
      backBtn.addEventListener('click', function () {
        vscode.postMessage({ type: 'show-list' });
      });

    const newBtn = document.getElementById('new-file');
    if (newBtn)
      newBtn.addEventListener('click', function () {
        vscode.postMessage({ type: 'new-file' });
      });
    ...
    if (reveal)
      reveal.addEventListener('click', function () {
        vscode.postMessage({ type: 'reveal-script' });
      });
    ...
    const expandableHeaders = document.querySelectorAll(
      'li.step.passed > .step-header, li.step.failed > .step-header, li.step.running > .step-header',
    );
